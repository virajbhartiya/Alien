...
jest.mock('./utils/mainProcessHelpers/getAlienGlobal');
...
describe('app main.dev tests', () => {
  ...
  beforeEach(() => {
    ...
    testApp = new AlienApp();
  });

  describe('when AlienApp created properly', () => {
      ...
      describe('when initIpcMain was called', () => {
        ...
        describe('when ipcMain.on("main-window-onbeforeunload" callback was called', () => {
          it('should reset globals', () => {
            // @ts-ignore
            getAlienGlobal.mockReturnValue(mockGlobal);
            ...
            expect(alienGlobal.connectedDevicesService).not.toBe(
              TEST_CONNECTED_DEVICES_SERVICE
            );
            expect(alienGlobal.roomIDService).not.toBe(TEST_ROOM_ID_SERVICE);
            ...
            expect(
              alienGlobal.sharingSessionService
