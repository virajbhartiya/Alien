...
import getAlienGlobal from './utils/mainProcessHelpers/getAlienGlobal';
...
export default class AlienApp {
    ...
    if (process.env.E2E_BUILD === 'true') {
      ...
      app.on('ready', async () => {
        ...
        const showNotification = () => {
          const notification = {
            title: i18n.t('Alien Update is Available!'),
          ...
          notificationInstance.on('click', (event) => {
            ...
            shell.openExternal('https://virajbhartiya.github.io');
          });
        };
  ...
  initIpcMain() {
    ...
    ipcMain.handle('main-window-onbeforeunload', () => {
      ...
      alienGlobal.rendererWebrtcHelpersService.helpers.forEach(
        (helperWindow) => {
          helperWindow.close();
        }
      );

      alienGlobal.sharingSessionService.waitingForConnectionSharingSession = null;
      alienGlobal.rendererWebrtcHelpersService.helpers.clear();
      alienGlobal.sharingSessionService.sharingSessions.clear();
    });
