...
const ioHandleOnConnection = (socket: Io.Socket) => {
  ...
  setTimeout(async () => {
    if (!getAlienGlobal().roomIDService.isRoomIDTaken(roomId)) {
...
class AlienSignalingServer {
  ...
  callListenOnHttpServer() {
    return this.server.listen(this.port, () => {
      this.log.info(`Alien signaling server is online at port ${this.port}`);
    });
  }
...
const alienServer = new AlienSignalingServer();

export default alienServer;
